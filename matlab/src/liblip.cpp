...
double defaultupperbound(int* dim, double* x, double* p) {return 10e20;}
double defaultlowerbound(int* dim, double* x, double* p) {return -10e20;}

USER_FUNCTION defup=&defaultupperbound;
USER_FUNCTION deflo=&defaultlowerbound;

class SLipIntInfB:public SLipIntInf {
public:
	virtual double  ExtraUpperBound(int dim, double* x, double * param) {return defup(&dim,x,param);};  // derived classes may overwrite these
	virtual double  ExtraLowerBound(int dim, double* x, double * param) {return deflo(&dim,x,param);};
};
class SLipIntB:public SLipInt {
public:
	virtual double  ExtraUpperBound(int dim, double* x, double * param) {return defup(&dim,x,param);};  // derived classes may overwrite these
	virtual double  ExtraLowerBound(int dim, double* x, double * param) {return deflo(&dim,x,param);};
};
...
void LipIntSetBounds(USER_FUNCTION low, USER_FUNCTION up)
{
	...
	slii.UseOtherBounds=1;
}

void LipIntClearBounds()
{
	sli.UseOtherBounds=0;
	slii.UseOtherBounds=0;
}
