...
{
    bool bRes = true;
    if ((!(mxIsDouble(prhs[paramNo]) || mxIsInt32(prhs[paramNo])) ) || mxGetN(prhs[paramNo])!= 1 ||  mxGetM(prhs[paramNo]) !=1 )
    {
...
bool parseCons(int nrhs, const mxArray *prhs[], sParams *pParams, int paramNo)
{
    ...
    if (!mxIsDouble(prhs[paramNo]) || !((mxGetN(prhs[paramNo])== pParams->m_Dim &&  mxGetM(prhs[paramNo])==1) || (mxGetM(prhs[paramNo])== pParams->m_Dim &&  mxGetN(prhs[paramNo])==1)))
    {
        ...
        mexPrintf("Do not use [int32(1), int32(1)], just use [1,1]\n");
        bRes = false;
    }
    else  //x check that below code works
    {
        pParams->m_pCons = /*(int *) */ mxGetPr(prhs[paramNo]);
		for(i=0;i<pParams->m_Dim;i++) MyCons[i]= (int) pParams->m_pCons[i];
	//	mexPrintf("constraints %d %d \n", pParams->m_pCons[0],pParams->m_pCons[1]);
...
bool parseY(int nrhs, const mxArray *prhs[], sParams *pParams, int paramNo)
{
    ...
    if (!mxIsDouble(prhs[paramNo]) || !((mxGetN(prhs[paramNo])>= pParams->m_N && mxGetM(prhs[paramNo])==1) ||  (mxGetM(prhs[paramNo])>= pParams->m_N && mxGetN(prhs[paramNo])==1) ))
    {
		...
		pParams->m_M1=min(mxGetM(prhs[paramNo]),pParams->m_N) ;
		pParams->m_N1=min(mxGetN(prhs[paramNo]),pParams->m_N);
	}
    return bRes;
}
...
{
    bool bRes = true;
    if (! (mxIsDouble(prhs[paramNo]) || mxIsInt32(prhs[paramNo])) )
    {
        mexPrintf("%d input must be an integer fW\n", paramNo);
        bRes = false;
    }
    else 
 ...
 //   if (!mxIsInt32(prhs[paramNo]) || mxGetN(prhs[paramNo])!= pParams->m_N ||  mxGetM(prhs[paramNo])!=1)
    {
        ...
        mexPrintf("Do not use [int32(1), int32(2)], just use [1,2], etc.\n");
        bRes = false;
    }
    else  //x check that below code works
    {
		MyIndex=(int*) malloc(sizeof(int)*pParams->m_N);
        pParams->m_pIndex = mxGetPr(prhs[paramNo]);
		for(i=0;i<pParams->m_N;i++) MyIndex[i]= (int) pParams->m_pIndex[i] -1;
...
double upwrap(int* dim, double* x, double* p) {
    ...
    memcpy(gParams.m_pDistParamArr_UB, x, *dim * sizeof(double));
	gParams.pParUB[0]= *p; 
...
double lowwrap(int* dim, double* x, double* p) {
    double res;
	if (!boundaries_set) return -10e20;
    memcpy(gParams.m_pDistParamArr_LB, x, *dim * sizeof(double));
	gParams.pParLB[0]= *p; 
...
bool parseParams(int nrhs, const mxArray *prhs[], sParams *pParams)
{
    ...
    if (bRes)
    {
        ...
        {       
            if ( nrhs == 10 || nrhs == 7 || nrhs == 8 || nrhs == 9)
            {
                bRes = parseDim(nrhs, prhs, pParams, paramNo++) && bRes;
                bRes = parseN(nrhs, prhs, pParams, paramNo++) && bRes;
                bRes = parseXd(nrhs, prhs, pParams, paramNo++) && bRes;
            ...
            {
                mexPrintf("Error: wrong number of parameters (%d). Must be  7 or 8 or 9 or 10.\n", nrhs);
                bRes = false;
            }
        }
        else  if (strcmp(pParams->m_pName, "smoothlipschitz") == 0 || strcmp(pParams->m_pName, "infsmoothlipschitz") == 0)
        {       
            if (nrhs == 9 || nrhs == 10 || nrhs == 11 || nrhs == 12 )
            {
                bRes = parseDim(nrhs, prhs, pParams, paramNo++) && bRes;
                bRes = parseN(nrhs, prhs, pParams, paramNo++) && bRes;
                bRes = parseXd(nrhs, prhs, pParams, paramNo++) && bRes;
                ...
                if (nrhs >=  paramNo  && pParams->m_fC)
                    bRes = parseCons(nrhs, prhs, pParams, paramNo++) && bRes;
                if (nrhs >=  paramNo  && pParams->m_fR!=0)
                    bRes = parseRegion(nrhs, prhs, pParams, paramNo++) && bRes;
            }
            else 
            {
                mexPrintf("Error: wrong number of parameters (%d). Must be 9 or 10 or 11 or 12.\n", nrhs);
                bRes = false;
            }
        }
...
//	debugParams(&gParams);
    if (bRes)
    {
        if (strcmp(gParams.m_pName, "value")==0)
        {
            ...
            pRes = mxGetPr(plhs[0]);
            *pRes = LipIntValue(&gParams.m_Dim, &gParams.m_N, gParams.m_pX, gParams.m_pXd, gParams.m_pYd, &gParams.m_LC, MyIndex);
        }
        ...
        if (strcmp(gParams.m_pName, "infvalue")==0)
        {
            plhs[0] = mxCreateDoubleMatrix(1,1,mxREAL);
            pRes = mxGetPr(plhs[0]);
            *pRes = LipIntInfValue(&gParams.m_Dim, &gParams.m_N, gParams.m_pX, gParams.m_pXd, gParams.m_pYd, &gParams.m_LC, MyIndex);
        }
        else if (strcmp(gParams.m_pName, "valueauto")==0)
        {
            plhs[0] = mxCreateDoubleMatrix(1,1,mxREAL);
            pRes = mxGetPr(plhs[0]);
            *pRes = LipIntValueAuto(&gParams.m_Dim, &gParams.m_N, gParams.m_pX, gParams.m_pXd, gParams.m_pYd, MyIndex);
        }
        else if (strcmp(gParams.m_pName, "infvalueauto")==0)
        {
            plhs[0] = mxCreateDoubleMatrix(1,1,mxREAL);
            pRes = mxGetPr(plhs[0]);
            *pRes = LipIntInfValueAuto(&gParams.m_Dim, &gParams.m_N, gParams.m_pX, gParams.m_pXd, gParams.m_pYd, MyIndex);
        }
        ...
        else if (strcmp(gParams.m_pName, "valuecons")==0)
        {
            plhs[0] = mxCreateDoubleMatrix(1,1,mxREAL);
            pRes = mxGetPr(plhs[0]);
            *pRes = LipIntValueCons(&gParams.m_Dim, &gParams.m_N, &MyCons[0], gParams.m_pX, gParams.m_pXd, gParams.m_pYd, &gParams.m_LC, MyIndex);
        }
        else if (strcmp(gParams.m_pName, "infvaluecons")==0)
        {
            plhs[0] = mxCreateDoubleMatrix(1,1,mxREAL);
            pRes = mxGetPr(plhs[0]);
            *pRes = LipIntInfValueCons(&gParams.m_Dim, &gParams.m_N, &MyCons[0], gParams.m_pX, gParams.m_pXd, gParams.m_pYd, &gParams.m_LC, MyIndex);
        }
        else if (strcmp(gParams.m_pName, "valueconsleftregion")==0)
        {
            plhs[0] = mxCreateDoubleMatrix(1,1,mxREAL);
            pRes = mxGetPr(plhs[0]);
            *pRes = LipIntValueConsLeftRegion(&gParams.m_Dim, &gParams.m_N, &MyCons[0], gParams.m_pX, gParams.m_pXd, gParams.m_pYd, &gParams.m_LC, gParams.m_pRegion, MyIndex);
        }
        else if (strcmp(gParams.m_pName, "infvalueconsleftregion")==0)
        {
            plhs[0] = mxCreateDoubleMatrix(1,1,mxREAL);
            pRes = mxGetPr(plhs[0]);
            *pRes = LipIntInfValueConsLeftRegion(&gParams.m_Dim, &gParams.m_N, &MyCons[0], gParams.m_pX, gParams.m_pXd, gParams.m_pYd, &gParams.m_LC, gParams.m_pRegion, MyIndex);
        }
        else if (strcmp(gParams.m_pName, "valueconsrightregion")==0)
        {
            plhs[0] = mxCreateDoubleMatrix(1,1,mxREAL);
            pRes = mxGetPr(plhs[0]);
            *pRes = LipIntValueConsRightRegion(&gParams.m_Dim, &gParams.m_N, &MyCons[0], gParams.m_pX, gParams.m_pXd, gParams.m_pYd, &gParams.m_LC, gParams.m_pRegion, MyIndex);
        }
        else if (strcmp(gParams.m_pName, "infvalueconsrightregion")==0)
        {
            plhs[0] = mxCreateDoubleMatrix(1,1,mxREAL);
            pRes = mxGetPr(plhs[0]);
            *pRes = LipIntInfValueConsRightRegion(&gParams.m_Dim, &gParams.m_N, &MyCons[0], gParams.m_pX, gParams.m_pXd, gParams.m_pYd, &gParams.m_LC, gParams.m_pRegion, MyIndex);
        }
        ...
        else if (strcmp(gParams.m_pName, "valuelocalcons")==0)
        {
            ...
            *pRes = LipIntValueLocalCons(&gParams.m_Dim, &gParams.m_N, &MyCons[0], gParams.m_pX, gParams.m_pXd, gParams.m_pYd);
        }
        else if (strcmp(gParams.m_pName, "infvaluelocalcons")==0)
        {
            plhs[0] = mxCreateDoubleMatrix(1,1,mxREAL);
            pRes = mxGetPr(plhs[0]);
            *pRes = LipIntInfValueLocalCons(&gParams.m_Dim, &gParams.m_N, &MyCons[0], gParams.m_pX, gParams.m_pXd, gParams.m_pYd);
        }
        else if (strcmp(gParams.m_pName, "valuelocalconsleftregion")==0)
        {
            plhs[0] = mxCreateDoubleMatrix(1,1,mxREAL);
            pRes = mxGetPr(plhs[0]);
            *pRes = LipIntValueLocalConsLeftRegion(&gParams.m_Dim, &gParams.m_N, &MyCons[0], gParams.m_pX, gParams.m_pXd, gParams.m_pYd, gParams.m_pRegion);
        }
        else if (strcmp(gParams.m_pName, "infvaluelocalconsleftregion")==0)
        {
            plhs[0] = mxCreateDoubleMatrix(1,1,mxREAL);
            pRes = mxGetPr(plhs[0]);
            *pRes = LipIntInfValueLocalConsLeftRegion(&gParams.m_Dim, &gParams.m_N, &MyCons[0], gParams.m_pX, gParams.m_pXd, gParams.m_pYd, gParams.m_pRegion);
        }
        else if (strcmp(gParams.m_pName, "valuelocalconsrightregion")==0)
        {
            plhs[0] = mxCreateDoubleMatrix(1,1,mxREAL);
            pRes = mxGetPr(plhs[0]);
            *pRes = LipIntValueLocalConsRightRegion(&gParams.m_Dim, &gParams.m_N, &MyCons[0], gParams.m_pX, gParams.m_pXd, gParams.m_pYd, gParams.m_pRegion);
        }
        else if (strcmp(gParams.m_pName, "infvaluelocalconsrightregion")==0)
        {
            plhs[0] = mxCreateDoubleMatrix(1,1,mxREAL);
            pRes = mxGetPr(plhs[0]);
            *pRes = LipIntInfValueLocalConsRightRegion(&gParams.m_Dim, &gParams.m_N, &MyCons[0], gParams.m_pX, gParams.m_pXd, gParams.m_pYd, gParams.m_pRegion);
        }
        ...
        else if (strcmp(gParams.m_pName, "computelipschitzcv")==0)
        {
            ...
            pRes = mxGetPr(plhs[0]);
            LipIntComputeLipschitzCV(&gParams.m_Dim, &gParams.m_N, gParams.m_pXd, gParams.m_pYd, pRes, &gParams.m_Type,&MyCons[0], gParams.m_pRegion, gParams.m_pW);
        }
        else if (strcmp(gParams.m_pName, "infcomputelipschitzcv")==0)
        {
            plhs[0] = mxCreateDoubleMatrix(gParams.m_M1, gParams.m_N1, mxREAL);
            pRes = mxGetPr(plhs[0]);
            LipIntInfComputeLipschitzCV(&gParams.m_Dim, &gParams.m_N, gParams.m_pXd, gParams.m_pYd, pRes, &gParams.m_Type, &MyCons[0], gParams.m_pRegion, gParams.m_pW);
        }
        else if (strcmp(gParams.m_pName, "computelipschitzsplit")==0)
        {
            plhs[0] = mxCreateDoubleMatrix(gParams.m_M1, gParams.m_N1, mxREAL);
            pRes = mxGetPr(plhs[0]);
            LipIntComputeLipschitzSplit(&gParams.m_Dim, &gParams.m_N, gParams.m_pXd, gParams.m_pYd, pRes, &gParams.m_Ratio, &gParams.m_Type, &MyCons[0], gParams.m_pRegion, gParams.m_pW);
        }
        else if (strcmp(gParams.m_pName, "infcomputelipschitzsplit")==0)
        {
            plhs[0] = mxCreateDoubleMatrix(gParams.m_M1, gParams.m_N1, mxREAL);
            pRes = mxGetPr(plhs[0]);
            LipIntInfComputeLipschitzSplit(&gParams.m_Dim, &gParams.m_N, gParams.m_pXd, gParams.m_pYd, pRes, &gParams.m_Ratio, &gParams.m_Type, &MyCons[0], gParams.m_pRegion, gParams.m_pW);
        }
        else if (strcmp(gParams.m_pName, "smoothlipschitz")==0)
        {
            plhs[0] = mxCreateDoubleMatrix(gParams.m_M1, gParams.m_N1, mxREAL);
            pRes = mxGetPr(plhs[0]);
            LipIntSmoothLipschitz(&gParams.m_Dim, &gParams.m_N, gParams.m_pXd, gParams.m_pYd, pRes, &gParams.m_LC, &gParams.m_fW, &gParams.m_fC, &gParams.m_fR, gParams.m_pW, &MyCons[0], gParams.m_pRegion);
        }
        else if (strcmp(gParams.m_pName, "infsmoothlipschitz")==0)
        {
            plhs[0] = mxCreateDoubleMatrix(gParams.m_M1, gParams.m_N1, mxREAL);
            pRes = mxGetPr(plhs[0]);
            LipIntInfSmoothLipschitz(&gParams.m_Dim, &gParams.m_N, gParams.m_pXd, gParams.m_pYd, pRes, &gParams.m_LC, &gParams.m_fW, &gParams.m_fC, &gParams.m_fR, gParams.m_pW, &MyCons[0], gParams.m_pRegion);
        }
        ...
        else if (strcmp(gParams.m_pName, "verifymonotonicity")==0)
        {
            plhs[0] = mxCreateDoubleMatrix(1, 1, mxREAL);
            pRes = mxGetPr(plhs[0]);
            *pRes = LipIntVerifyMonotonicity(&gParams.m_Dim, &gParams.m_N, &MyCons[0], gParams.m_pXd, gParams.m_pYd, &gParams.m_LC, &gParams.m_eps);
        }
        else if (strcmp(gParams.m_pName, "infverifymonotonicity")==0)
        {
            plhs[0] = mxCreateDoubleMatrix(1, 1, mxREAL);
            pRes = mxGetPr(plhs[0]);
            *pRes = LipIntInfVerifyMonotonicity(&gParams.m_Dim, &gParams.m_N, &MyCons[0], gParams.m_pXd, gParams.m_pYd, &gParams.m_LC, &gParams.m_eps);
        }
        else if (strcmp(gParams.m_pName, "verifymonotonicityleftregion")==0)
        {
            plhs[0] = mxCreateDoubleMatrix(1, 1, mxREAL);
            pRes = mxGetPr(plhs[0]);
            *pRes = LipIntVerifyMonotonicityLeftRegion(&gParams.m_Dim, &gParams.m_N, &MyCons[0], gParams.m_pXd, gParams.m_pYd, gParams.m_pRegion,&gParams.m_LC, &gParams.m_eps);
        }
        else if (strcmp(gParams.m_pName, "infverifymonotonicityleftregion")==0)
        {
            plhs[0] = mxCreateDoubleMatrix(1, 1, mxREAL);
            pRes = mxGetPr(plhs[0]);
            *pRes = LipIntInfVerifyMonotonicityLeftRegion(&gParams.m_Dim, &gParams.m_N, &MyCons[0], gParams.m_pXd, gParams.m_pYd, gParams.m_pRegion,&gParams.m_LC, &gParams.m_eps);
        }
        else if (strcmp(gParams.m_pName, "verifymonotonicityrightregion")==0)
        {
            plhs[0] = mxCreateDoubleMatrix(1, 1, mxREAL);
            pRes = mxGetPr(plhs[0]);
            *pRes = LipIntVerifyMonotonicityRightRegion(&gParams.m_Dim, &gParams.m_N, &MyCons[0], gParams.m_pXd, gParams.m_pYd, gParams.m_pRegion,&gParams.m_LC, &gParams.m_eps);
        }
        else if (strcmp(gParams.m_pName, "infverifymonotonicityrightregion")==0)
        {
            plhs[0] = mxCreateDoubleMatrix(1, 1, mxREAL);
            pRes = mxGetPr(plhs[0]);
            *pRes = LipIntInfVerifyMonotonicityRightRegion(&gParams.m_Dim, &gParams.m_N, &MyCons[0], gParams.m_pXd, gParams.m_pYd, gParams.m_pRegion,&gParams.m_LC, &gParams.m_eps);
        }
